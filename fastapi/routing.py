...
def add_api_route(
    ...
    *,
    ...
    route_class = route_class_override or APIRoute
    logging.info(f"Adding API route: {path}")
    logging.debug(f"Endpoint: {endpoint.__name__}")
    logging.debug(f"Methods: {methods}")
    ...
    logging.debug(f"Response description: {response_description}")
    logging.debug(f"Responses: {responses}")
    logging.debug(f"Deprecated: {deprecated}")
    logging.debug(f"Include in schema: {include_in_schema}")
    logging.debug(f"Response class: {response_class}")
    logging.debug(f"Name: {name}")
    logging.debug(f"Route class override: {route_class_override}")
    logging.debug(f"Callbacks: {callbacks}")
    logging.debug(f"OpenAPI extra: {openapi_extra}")
    logging.debug(f"Generate unique ID function: {generate_unique_id_function}")
    logging.debug(f"Strict content type: {strict_content_type}")
    super().add_api_route(
        path,
        endpoint,
